...
async def msg_build(ctx, *args):
    ...
    await ctx.channel.send(
        content=deck_list.to_vdb(), file=discord.File(fp=deck_file, filename=deck_name)
    )
    deck_file.close()
...
async def msg_top(ctx, *, args: parser.vtes = parser.vtes.defaults()):
    ...
    for k, v in args.items():
        b = v in search.search_dimensions[k]
        print(f"key **{k}**: value `{v}` // exists {b}")
        if b:
            test = {k: v}
...
async def msg_deck(ctx, *, args: parser.twda = parser.twda.defaults()):
    ...
    authors = [
        _u.normalize(a) for a in authors if _u.normalize(a) in twda.TWDA.by_author
    ]
    ...
    for d in sorted(decks, key=lambda a: a.date):
        if full:
            ...
            await ctx.channel.send(
                content=output, file=discord.File(fp=deck_file, filename=d.id)
            )
            deck_file.close()
        else:
